            ...
            <table class="table datatable-basic table-items">
                ...
                @foreach ($data['news'] as $news)
                    <tr role="row" id="{{ 'news_'.$news->id }}">
                        <td class="image"><a class="img-preview" href="{{ asset('images/news/'.$news->image) }}"><img src="{{ asset('images/news/'.$news->image) }}" /></a></td>
                        <td class="text-left head">{{ $news->head }}</td>
                        <td class="text-left">@include('admin.blocks._cropped_content_block',['croppingContent' => $news->text, 'length' => 300])</td>
